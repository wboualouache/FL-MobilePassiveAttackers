...
void TestWaveApplLayer::onBeacon(WaveShortMessage* wsm) {
    ...
    attacker.close();
	   ...
	   // The type of this attacker is attack-and-stop attacker. It changes its statues every 30s
	   if (((int)(cur_time.dbl()/30) %2==0)){
        if(type==1){
        ...
        traci->commandSetSpeed(currentspeed+1);
            ...
            traci->commandSetSpeed(18); // In non-attack case its keep ordinary speed 18m/s, which is less than the speed of the victim
        }
        }
    }
    }
